...
class UNetTrainerGUI:
    def __init__(self, master):
        ...
        self.master = master
        master.title("基于unet算法的矿石图像分割图形化界面")
        ...
        self.model_file_label = ttk.Label(master, text="选择权重：")
        self.model_file_label.grid(row=2, column=0, padx=10, pady=10)
        self.model_file_button = ttk.Button(master, text="选择", command=self.choose_model)
        ...
        self.test_begin_button = ttk.Button(master, text="开始测试", command=self.predict)
        ...
        # 创建“跳出”按钮
        self.stop_button = tk.Button(self.master, text='Stop',command=self.stop_train)
